...
# copied from above from test.py and the default values from sda\test.py in the parse_test_config()
...
def detect_objects(input_bev_maps, model, configs):
    ...
    if len(detections) > 0:
        ...
        for ele in detections:
            id, x_bv, y_bv, z_bv, h_bv, w_bv, l_bv, yaw = ele

            ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure

            x = y_bv / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0]) + configs.lim_x[0]
            y = x_bv / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0]) + (configs.lim_y[0])
            w = w_bv / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0])
            l = l_bv / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0])
            z = z_bv + configs.lim_z[0]
            #z = z_bv
            ## step 4 : append the current object to the 'objects' array
            #Check if bounding box is correct
            if ((y >= configs.lim_y[0] and y <= configs.lim_y[1]) and (x >= configs.lim_x[0] and x <= configs.lim_x[1]) and  (z >= configs.lim_z[0]
                and z <= configs.lim_z[1])):
                     objects.append([1, x, y, z, h_bv, w, l, -yaw])
