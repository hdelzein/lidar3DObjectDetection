...
def show_pcl(pcl):
    ...
    def arrow_click(vis):
        #vis.capture_screen_image('point_cloud.jpg')
        vis.destroy_window()

    vis = o3d.visualization.VisualizerWithKeyCallback()
    vis.register_key_callback(262, arrow_click)
    vis.create_window(width=1000, height=500, left=100, top=40, visible=True)
...
def bev_from_pcl(lidar_pcl, configs, show_image = False):
    ...
    discret_bev = (configs.lim_x[1] - configs.lim_x[0]) / configs.bev_height

    ## step 2 : create a copy of the lidar pcl and transform all metrix x-coordinates into bev-image coordinates
    lidar_pcl_cpy = np.copy(lidar_pcl)
    lidar_pcl_cpy[:, 0] = np.int_(np.floor(lidar_pcl_cpy[:, 0] / discret_bev))


    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    y_offset = (configs.bev_width + 1) / 2
    lidar_pcl_cpy[:, 1] = np.int_(np.floor(lidar_pcl_cpy[:, 1] / discret_bev) + y_offset)
    # similair to the excersice   ri[ri<0]=0.0
    ...
    # Fist step similiar to the excersice is set the max intensity to 1`
    ...
    if show_image:
        ...
        #cv2.imwrite("intensity_img.jpg", img_intensity)
        cv2.waitKey(0)
    ...
    if show_image:
        ...
        #cv2.imwrite("height_img.jpg", img_height)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
